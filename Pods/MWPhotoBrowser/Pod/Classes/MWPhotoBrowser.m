...
- (void)updateNavigation {
    ...
    MWPhoto *photo = [self photoAtIndex:_currentPageIndex];
#pragma mark -video下显示actionButton
...
#pragma marl -hideController showController
...
#endif
    UIActionSheet *actionSheet=[[UIActionSheet alloc]initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"删除" otherButtonTitles:@"保存到相册", nil];
...
{
    if (buttonIndex==1) {
        NSIndexSet *indexes =_selectedIndexes;
        if ([self.delegate respondsToSelector:@selector(photoBrowser:savePhotosAtIndexes:)]) {
            [self.delegate photoBrowser:self savePhotosAtIndexes:indexes];
        }
    }
    if (buttonIndex == actionSheet.destructiveButtonIndex) {
        
        NSIndexSet *indexes =_selectedIndexes;
        if ([self.delegate respondsToSelector:@selector(photoBrowser:deletePhotosAtIndexes:)]) {
            [self.delegate photoBrowser:self deletePhotosAtIndexes:indexes];
        }
    }
}
